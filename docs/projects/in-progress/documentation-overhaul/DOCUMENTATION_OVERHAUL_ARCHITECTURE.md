...
- **Key Concepts**:
  - Global Vault: `~/.agent/memory-vault/` containing broad developer preferences and identity keys (e.g. USER.md, SOUL.md).
  - Project Vault: `<repo>/.agent/memory-vault/` containing repository-specific facts and patterns.
...
- **Key Concepts**:
  ...
  - Dual-layer initialization: How `npx total-recall init` provisions global config in `~/.agent/` and project-level configs in the active repository.
