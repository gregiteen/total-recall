...
### 🐛 Bug Fixes
- **Every spawned agent received a Gemini key that could not call Gemini, because a stale legacy store outranked the brain.** `callLocalRuntime` walked four `secrets.enc` locations and took the **first one holding any keys**, in legacy-first order — so a 12-key `<cwd>/.agent/secrets.enc` shadowed the 100-key brain store entirely and the remaining layers were never read. Its `google_api_key` was a dead `AIza…` value returning **HTTP 400** from `generativelanguage`, while the working key sat unreachable one layer down. Layers are now merged in explicit precedence — legacy flat stores lowest, then global brain, then project brain (matching `resolveBrainDir('auto')`) — so a legacy file can still supply keys nothing else has but can no longer mask a canonical store. Four dead `AIza…` copies were replaced with the live key across the project brain and both legacy stores; all now return **HTTP 200**. Extracted as `loadDynamicSecrets()` so the layering is testable.
- **The research agenda reported 231 failures that were never attempted.** Both agenda-cleanup paths retired topics by writing `status: 'failed'` and a `cancelled_reason`, so any tally of `status === 'failed'` showed 231 phantom failures beside 44 genuinely pending topics — and `research-queue.mjs` never read `cancelled_reason` at all. Cancellation now has its own `cancelled` status (`CANCELLED_STATUS`), `cancelBogusAgendaTopics()` uses it, and the idempotent `reclassifyCancelledAgendaTopics()` repairs rows written before it existed. `getNextAgendaTopic()` already filtered to `pending | partially-covered`, so cancelled topics were never selectable — that is now asserted rather than assumed. The live agenda went from `failed: 231` to `cancelled: 231, failed: 0`.
- **`daemon stop` reported success while the daemon kept running the old code.** The SIGTERM handler cleared `running`, but the vault and source watchers hold the event loop open, so the process lingered indefinitely — "Daemon stopped." appears nowhere in `daemon.log` across three separate SIGTERMs, and the node kept ingesting sessions. Worse, the handler released the PID lock while still alive, so the server started a **second** daemon alongside the first. Shutdown is now a single `shutdown()` that stops the watchers, releases the lease, and arms a bounded 30 s fallback before the loop exits with an explicit `process.exit(0)`. Verified live: SIGTERM 06:24:04 → "Daemon stopped." 06:24:07 → process gone.
- **A node that could not run Total Recall could capture leadership and silence every real machine.** The `TR_LEADER_IP` pin exists because lowest-online-IP election gave the shared droplet (which runs no TR) permanent leadership, leaving every other node a follower that skipped the task loop. But the pin lived only in the supervisor's environment, so `total-recall daemon start` from a plain shell silently dropped it and the node became a follower again — the same outage, invisible. `leaderPin()` now falls back to `<brain>/config/leader.json`, and the daemon logs a loud warning when it follows with no pin configured anywhere, naming both ways to set one.
- **The shipped `route-manifest.json` described a server that no longer existed, and four spec files could only pass on one developer's machine.** The manifest was last generated on 2026-08-20 while the router gained 12 routes on 2026-09-15 (`/api/mesh/dispatch`, `/api/mesh/presence`, `/api/mesh/presence/local`, `/api/mesh/presence/resolve`, and the plugin routes), so `route-inventory.spec.mjs` compared 233 live routes against 221 committed ones. Regenerated. Separately, `plugin-loader.spec.mjs` and `plugin-context.spec.mjs` read a hard-coded `/Users/greg/Documents/antigravity/keen-hertz` and asserted on a plugin that is gone even there; they now build a self-contained temp fixture. `search.spec.mjs` asserted a `searchPlugins` export that no commit ever added — it now covers the real `searchCatalog` surface. `App.spec.tsx` and `PluginsPage.spec.tsx` failed on a mock missing `fetchPlugins` and on `useSearchParams()` rendered outside a Router. The suite is green end to end again.
...
### ✨ Features
- **Watch mode: arm the server to approve the next device.** Pairing was built backwards — it minted a key, drew a QR code nobody can scan (no Tailscale client scans a QR to authenticate; tailscale#8267, #13377 and #14184 are still open requests), and asked you to carry a 40-character secret from a laptop to a phone by hand. iOS cannot use a pre-auth key against a custom control server at all, so for the one device that most needs pairing the whole flow was a dead end. The working direction is the reverse: the device generates a registration id and waits, and the server approves it. As a button that is one click — arm it, sign in on the phone, and it connects with nothing typed or copied anywhere. headscale exposes no pending-registration API (the entry lives in an in-memory cache, visible only in the server log), so watch mode needs a log source and reports `unavailable` where none is configured rather than offering a button that cannot work. The log command comes from configuration — `TR_HEADSCALE_LOG_CMD`, or `headscale_log_command` on the stored key — and no host is named anywhere.

### 🐛 Bug Fixes
- **The pairing card drew a QR code for an address nothing was listening on, and reported no problem.** It worked the URL out by derivation — loopback, plus whatever `getMeshIp()` said when the card was opened — and every reachability check downstream read that guess. On a brain bound to loopback alone it therefore recommended the mesh URL, rendered a scannable code for it, and stayed silent. One machine served that dead code for a week. `/api/pairing` now reports the sockets the process actually bound, with `listen_hosts_source`, per-endpoint `listening`, and a plain statement when no other device can reach the brain at all. The card greys out the dead code and names the cause and the fix instead of leaving you to find them.
- **Every install without a `security.yml` bound loopback only — permanently, and silently.** `loadSecurityConfig()` defaulted to `bind: { host: '127.0.0.1' }`, and `resolveServerHost()` gives a configured host precedence over the mesh address; it cannot tell that default apart from a user who typed the address deliberately. So a mesh address was discovered and then discarded, `usedLoopbackFallback` stayed false, and even the existing warning never fired. The default no longer pins `bind.host`. An explicit choice still wins, and `allow_public_bind` still gates `0.0.0.0`.
- **A brain that lost the boot race against the mesh client stayed invisible until someone restarted it by hand.** `getMeshIp()` genuinely returns null at launchd startup, and the bind was resolved once and never revisited. A loopback-only brain now watches for a mesh address and binds it if one appears, so the race self-heals. Observed on a real node: *"Bound to loopback only"* at 16:54:44, *"Mesh address appeared after startup"* at 16:54:59.
- **First-run setup could be claimed by any device on the tailnet.** `POST /auth/setup` cannot require authentication — on an unconfigured brain there is no credential to present — so its only guard was "no password set yet", which makes *who may call it* the entire access control. That was safe purely because a config-less brain bound loopback only; the bind fix above removes that accident, and the first device to POST would have owned the brain. The route now requires a local request, and the refusal names the way out rather than dead-ending a headless host: `npx total-recall reset-password`. The configured-ness check stays ahead of the locality check on purpose, so an already-claimed brain gives every caller the same answer and never confirms that "unclaimed" was once true.
