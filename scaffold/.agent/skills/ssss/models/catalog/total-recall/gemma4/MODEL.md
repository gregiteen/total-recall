...
description: >-
  Sovereign local Gemma 4 26B running via Ollama on the user's own hardware.
...
This model file registers a locally-running Gemma 4 26B instance as a selectable model in UltraChat's model catalog.
...
3. **UltraChat discovers this MODEL.md** file via VFS sync and adds the brain to the model selector.
4. **Users select it** like any other model — no API key, no per-token billing.
...
UltraChat model catalog services should:
...
4. Show `pricing` as "$0.00" (sovereign, no per-token cost).
5. Set `sovereign: true` flag to indicate the model runs on user hardware.
