...
- **ABSOLUTE PURGE**: Deleted over 150 lines of strictly obsolete legacy MCP laws (Law #3, Law #-1, Legacy MCP generators, and VS Code MCP activations).
- **ENFORCED**: Zero MCP Servers explicitly documented in Law #-7. This is a Code-Mode only environment.
...
- **MANDATED**: `mcp_filesystem` tools for all searching.
...
- **MANDATED** `mcp_filesystem_find_by_name` for all search operations.
- **REASON**: The internal `fd` binary provided by the agent environment is incompatible with the system's CPU architecture (Bad CPU Type), causing persistent crashes. Using the MCP version (which wraps native `find`) solves this.

## [1.0.0] - 2026-01-17

- Initial skill creation for cross-IDE instruction management.
- Implementation of `INSTRUCTION_LOG.md` protocol.
2026-03-06 08:49:54 - Added LAW #-6 forbidding automatic dev server execution
