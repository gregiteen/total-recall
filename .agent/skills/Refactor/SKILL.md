...
Before calling `/switch` to hand a refactor to a fresh session, you MUST capture the following in `HANDOFF.md`:
...
The fresh session must run `/start` first, then read `HANDOFF.md` to find this section.
...
| File type | Example |
|---|---|
| Monolithic daemon files | `surface.mjs` into modular routing modules |
| Test files | `*.test.ts`, `*.spec.ts` |
| VFS Schema definition files | `.agent/memory-vault/schema.json` |
...
- **Pre-Switch Handoff Protocol**: Formal section capturing what must be in HANDOFF.md before calling `/switch`
