...
- ALWAYS use the relevant skill for every task and ANNOUNCE which skill you are using at the start of the task. Read the SKILL.md file before executing. Skills exist for a reason — they contain rules, protocols, and standards. Never bypass them or work ad-hoc when a skill covers the task domain.
- EVERY project, epic, or multi-step task MUST use the /project-management skill with proper docs/projects/ tracker files (PROJECT_TRACKER.md, DEV_PLAN.md). NEVER use transient artifact implementation plans for project work. All project documentation lives in docs/projects/ organized as a Kanban board (in-progress/, completed/, archived/, planned/, backlog/).
- NEVER implement a --no-backup, --skip-backup, noBackup, skipBackup, or ANY flag that allows skipping the backup step in the uninstall command. A previous AI agent added a --no-backup flag and it caused CATASTROPHIC DATA LOSS of the user's entire memory vault (brain). The backup step before uninstall is MANDATORY and NON-NEGOTIABLE. If backup fails, uninstall MUST abort. There is a runtime guard in src/cli/uninstall.mjs that rejects these flags — do NOT remove it. There are NO exceptions to this rule.
...
- When explaining code flow or presenting work to the user, NEVER present incomplete or non-compliant work as if it follows the rules. If you haven't read the relevant SKILL.md, say so. If you skipped a step, say so. Presenting sloppy work as correct is dishonest. Always read the SKILL.md FIRST, follow ALL rules, and be transparent about any gaps.
- When explaining code flow or logic to the user, ALWAYS proactively flag gaps, missing edge cases, and uncovered branches you notice. Do NOT present incomplete logic as if it is complete. If you see a conditional chain with a missing else clause or an unhandled case, call it out immediately — do not wait for the user to discover it themselves. Presenting incomplete code as complete is dishonest and wastes the user's time.
